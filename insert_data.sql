...
BEGIN
	SELECT od.store_id, SUM(DISTINCT od.total_price) AS total_sales, 	MONTH(od.date) AS sale_month, YEAR(od.date) AS sale_year
	FROM order_details od
	WHERE MONTH(date) = month_param AND YEAR(date) = year_param
	GROUP BY od.store_id, MONTH(od.date), YEAR(od.date)
	ORDER BY SUM(DISTINCT od.total_price) DESC;
END $$
DELIMITER ;
...
BEGIN
	SELECT SUM(DISTINCT od.total_price) AS total_sales, MONTH(od.date) AS sale_month, YEAR(od.date) AS sale_year
	FROM order_details od
	WHERE MONTH(date) = month_param AND YEAR(date) = year_param
